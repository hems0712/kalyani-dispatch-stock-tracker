...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const dispatchVehicle = (vehicleKey: 'v1Load' | 'v2Load' | 'v3Load') => {
    ...
    setStocks(prev => prev.map(s => {
      ...
      if (loadQty > 0) {
        ...
        const updates = { shippedQuantity: (Number(s.shippedQuantity) || 0) + loadQty, [shippedKey]: (Number(s[shippedKey]) || 0) + loadQty, [vehicleKey]: 0 };
        updatesList.push({ partNumber: s.partNumber, updates });
        return { ...s, ...updates };
      }
      return s;
  ...
  const recallVehicle = (vehicleKey: 'v1Load' | 'v2Load' | 'v3Load') => {
    ...
    setStocks(prev => prev.map(s => {
      ...
      if (shippedQty > 0) {
        const updates = { shippedQuantity: Math.max(0, (Number(s.shippedQuantity) || 0) - shippedQty), [shippedKey]: 0, pdiStock: Number(s.pdiStock) + shippedQty };
        updatesList.push({ partNumber: s.partNumber, updates });
        return { ...s, ...updates };
      }
      return s;
